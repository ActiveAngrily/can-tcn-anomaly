"""
Vehicular Anomaly Detection Dashboard

This module subscribes to a ROS topic streaming CAN data, normalizes it using a sliding
window preprocessor, and feeds it into a pre-trained Temporal Convolutional Network (TCN).
It visualizes the current anomaly risk score in the terminal.
"""
import torch
import numpy as np
import rospy
...
# --- CONFIGURATION ---
# The threshold above which an anomaly is triggered (determined during validation)
THRESHOLD = 0.3101 
...
try:
    # Prefer JSON scaler (for cross-platform compatibility), fallback to Joblib
    ...
    try:
        ...
        # INFERENCE PHASE
            ...
            with torch.no_grad():
                ...
                # Output the result back to the user interface
                # Added flush=True to force the bar to appear immediately
                # Added spaces at the end to clear any leftover characters from previous prints
